...
onMounted(async () => {
  ...
  if (!novelas.value.length) return

  novela.value = novelas.value[0]

  await cargarCapitulos()

  capitulosNovela.value =
    obtenerCapitulosByNovela(novela.value.id)
})
</script>

<template>
  <NovelLanding v-if="novela" :title="novela.titulo" :author="novela.autor" :status="novela.estado"
    :coverImage="novela.imagen" :synopsis="novela.info.sinopsis" :tags="novela.tags" :chapters="capitulos" />
</template>